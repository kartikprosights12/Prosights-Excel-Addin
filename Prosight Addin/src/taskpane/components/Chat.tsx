...
const Chat: React.FC = () => {
  ...
  const addMessage = async (text: string, type: "question" | "explanation") => {
    if (type === "question") {
      ...
      // Check if the input text contains the word "load" (case-insensitive)
      console.log(text);
      console.log(text.toLowerCase().includes("build"));
  ...
  const demoFunction = async () => {
    Excel.run(function (context) {
      ...
      sheet.getRange("D8").values = [["10"]];
      sheet.getRange("D9").values = [["$1,500"]];
      sheet.getRange("D10").values = [["0.0%"]];
      sheet.getRange("D11").values = [["70.0%"]];
      sheet.getRange("D13").values = [["$75/rest."]];
      sheet.getRange("D14").values = [["$500/own."]];
      sheet.getRange("D15").values = [["0.5%"]];
      sheet.getRange("D16").values = [["$300/rest."]];
      sheet.getRange("D17").values = [["$10/rest."]];
      sheet.getRange("D18").values = [["$650/rest."]];
      sheet.getRange("D8:D18").format.font.color = "blue";
